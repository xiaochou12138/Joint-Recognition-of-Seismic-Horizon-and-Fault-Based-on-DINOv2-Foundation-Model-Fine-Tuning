...
from demo_classification_two_tasks import get_args
import os
...
if __name__ == '__main__':
    ...
    model_Path = 'checkpoint/'+ args.dataset2 +'/'+ args.loss +'/'+args.netType_fault +'/'+args.checkpointName + "_" + args.vt+'_minloss_train.pth'
    lora_Path = 'checkpoint/'+ args.dataset2 +'/'+ args.loss +'/'+args.netType_fault +'/'+args.checkpointName + "_" + args.vt+'_minloss_train_lora.pth'
    pngPath = 'png/'+ args.dataset2 +'/'+ args.netType_fault +'/'+ args.loss +'/' +args.checkpointName + "_" + args.vt+'/'
    ...
    data, predict, target = goPredict(net, device, patch_h, patch_w, model_Path, lora_Path ,args.dataset2, args.netType_fault)
    for i in range(len(data)):    
        ...
        plt.figure(figsize=[10,10])
        plt.imshow(predict[i][0], cmap='jet')
        plt.show()

        plt.figure(figsize=[10,10])
        plt.imshow(target[i][0], cmap='jet')
        plt.show()